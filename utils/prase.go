...
func BindForm(req *http.Request, obj interface{}) error {
	...
	switch obj.(type) {
	case *template.User:
		obj.(*template.User).Mobile = req.Form.Get("mobile")
		obj.(*template.User).Nickname = req.Form.Get("nickname")
		obj.(*template.User).Password = req.Form.Get("passwd")
		obj.(*template.User).Avatar = req.Form.Get("avatar")
		obj.(*template.User).Memo = req.Form.Get("memo")
		obj.(*template.User).Sex = req.Form.Get("sex")
	case *template.Contact:
	case *template.Community:

	}
	return nil
}

func BindFormV2(req *http.Request, ptr interface{}) error {
	_ = req.ParseForm()
